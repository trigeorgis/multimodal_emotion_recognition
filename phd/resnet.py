...
def inference(x,lands, is_training,num_units_out_fc1=1500,num_units_out_fc2=256,
              num_classes=8,
    ...
    with tf.variable_scope('fc1'):
      x = fc1(x, lands,c,is_training,num_units_out_fc1)

    with tf.variable_scope('fc2'):
      x = fc2(x, c,is_training,num_units_out_fc2)

    with tf.variable_scope('fc'):
      x = fc(x, c,is_training)
...
def fc1(x,lands, c,is_training,num_units_out_fc1):
    num_units_in = x.get_shape()[1] + 68*2
    ...
    lands = tf.reshape(lands,[-1,68*2])
    x = tf.concat(1,[x,lands])
...
def fc2(x, c,is_training,num_units_out):
    num_units_in = x.get_shape()[1] 

    weights_initializer = tf.truncated_normal_initializer(
        stddev=FC_WEIGHT_STDDEV)

    weights = _get_variable('weights_fc2',
                            shape=[num_units_in, num_units_out],
                            initializer=weights_initializer,
                            weight_decay=FC_WEIGHT_STDDEV,trainable = is_training)
    biases = _get_variable('biases_fc2',
                           shape=[num_units_out],
                           initializer=tf.zeros_initializer,trainable = is_training)    
...
def fc(x, c,is_training):
    num_units_in = x.get_shape()[1]
    num_units_out = c['fc_units_out']
    weights_initializer = tf.truncated_normal_initializer(
        stddev=FC_WEIGHT_STDDEV)

    weights = _get_variable('weights_fc',
                            shape=[num_units_in, num_units_out],
                            initializer=weights_initializer,
                            weight_decay=FC_WEIGHT_STDDEV,trainable = is_training)
    biases = _get_variable('biases_fc',
                           shape=[num_units_out],
                           initializer=tf.zeros_initializer,trainable = is_training)
    x = tf.nn.xw_plus_b(x, weights, biases)
    return x
