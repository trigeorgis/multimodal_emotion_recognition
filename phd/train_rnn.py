...
fld = '/vol/atlas/homes/dk15/phd/rnn'+k+'epochs='+str(epochs)+'_same='+str(FLAGS.same)+'_different='+str(FLAGS.different)+'_hidden_units='+str(FLAGS.hidden_units)+'_lr='+str(initial_learning_rate)+'_fc1='+str(FLAGS.num_units_out_fc1)
...
def run_training(scope=''):
  with tf.Graph().as_default():  
    ...
    image_batch, label_batch,land_batch = tf.train.shuffle_batch(
    ...
    cnn_output = resnet.inference(image_batch,land_batch,True,FLAGS.num_units_out_fc1)    
    ...
    if FLAGS.summaries == 1:
      ...
      summaries.append(tf.histogram_summary('landmarks', land_batch))
