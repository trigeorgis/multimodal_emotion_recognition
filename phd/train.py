...
if FLAGS.person_ind == 1:
  ...
  input_land = '/vol/atlas/homes/dk15/correct_person_independent_train_lands.txt'             ## 902632 landmarks
  k = '/person_independent/'
else:
  inputt = '/homes/dk15/Desktop/labels4annotators.txt'                           ## 810880 images
  input_land = '/homes/dk15/Desktop/lands4annotators.txt'                        ## 810880 landmarks
  k ='/' 

fld = '/vol/atlas/homes/dk15/phd'+k+'epochs='+str(epochs)+'_batch_size='+str(FLAGS.batch_size)+'_lr='+str(initial_learning_rate)+'_fc1='+str(FLAGS.num_units_out_fc1)+'_fc2='+str(FLAGS.num_units_out_fc2)
...
def run_training(scope=''):
  with tf.Graph().as_default():  
    ...
    input_queue = tf.train.slice_input_producer([images, labels,lands],num_epochs=None, shuffle=True, seed=None,capacity=10000, shared_name=None, name=None)
    ...
    predictions = resnet.inference(image_batch,land_batch,True,FLAGS.num_units_out_fc1,FLAGS.num_units_out_fc2)    
