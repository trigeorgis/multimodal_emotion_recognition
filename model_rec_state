...
def audio_model(inputs, hidden_units=128, conv_filters=40):
    ...
    with slim.arg_scope([slim.layers.conv2d], padding='SAME'):
        ...
        # state is tuple = false, giati alliws mou ekane tin zwi duskoli
        
        stacked_lstm = tf.nn.rnn_cell.MultiRNNCell([lstm] * 2, state_is_tuple=False)
        # variable giati theloume na allazei se kathe batch 
        init_state = tf.Variable(stacked_lstm.zero_state(batch_size, tf.float32), trainable=False)

        net = tf.reshape(net, (batch_size, seq_length, num_features // 2 * 4))
        outputs, states = tf.nn.dynamic_rnn(stacked_lstm, net,initial_state=init_state, dtype=tf.float32)
        # put as the next initial state the output of the previously computed batch
        with tf.control_dependencies([init_state.assign(states)]):   
           outputs = tf.identity(outputs)

        net = tf.reshape(outputs, (batch_size * seq_length, hidden_units))
        net = slim.dropout(net)
        return tf.reshape(slim.layers.linear(net, 2), (batch_size, seq_length, 2)),states
